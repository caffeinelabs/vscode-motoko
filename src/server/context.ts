...
// Copied by `npm run prepare:moc-bundled`; loaded fresh per cache key.
...
/**
 * Build a `moc.js` instance in its own module scope. `require()` (and jest's
 * module registry) returns a singleton whose `Flags` state would be shared
 * across every `Context`, leaking per-canister flags (e.g.
 * `--enhanced-migration`) globally. Cached by the caller via `motokoInstances`.
 */

...
/**
 * Create or reuse a `moc.js` compiler instance. Each unique cache key gets
 * its own instance with independent `Flags` state, which is what canister
 * contexts (with per-canister flags like `--enhanced-migration`) rely on.
 */
...
async function createMotokoInstance(
    ...
    function getCompiler(path: string): Result<[any, string], Error> {
        try {
            const compiler = loadFreshCompiler(path);
...
function requestDefaultContext() {
    if (!defaultContext) {
        // Use the shared npm `motoko` singleton so tests that import it
        // directly (e.g. syntax.spec.ts) see the same compiler state.
        const motoko = defaultMotoko;

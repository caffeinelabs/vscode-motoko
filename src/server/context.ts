...
// Path to the bundled `moc.min.js`, copied by `npm run prepare:moc-bundled`.
// Used to spawn isolated compiler instances (one per canister) so that
// per-canister flags like `--enhanced-migration` don't leak across files.
...
async function createMotokoInstance(
    ...
    return [
        require('motoko/lib').default(loadFreshCompiler(bundledMocJsPath)),
        { source: 'bundled' },
    ];
...
function requestDefaultContext() {
    if (!defaultContext) {
        ...
        const mocJsInfo: MocJsInfo = { source: 'bundled' };
        configureMotokoCompiler(motoko);
